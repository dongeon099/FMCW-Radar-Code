...
# [그래프 고정] 화면 좌표 범위
VIEW_X_MIN, VIEW_X_MAX = -1.0, 1.0
VIEW_Y_MIN, VIEW_Y_MAX = 0.0, 1.0
...
def dbscanScattering(points):
    points = np.array(points, dtype=float)

    distance = np.sqrt(points[:, 0]**2 + points[:, 1]**2 + points[:, 2]**2)
    
    #거리 필터링 코드 
    valid = (distance >= 0.00) & (distance <= 5) # 논리회로(boolean logic) distance는 넘파이 행렬이므로, & 사용시 행들마다 true fallse로 즉 참, 거짓으로 valid에 저장 
    points = points[valid] #참 거짓으로 저장된 열벡터를 points 넘파이 행렬에 넣어 true인 부분만 빼서 다시 points에 저장 
...
def cluster(points, labels):
    "클러스터링 코드, dbscan으로 라벨링된 포인트의 중심점을 잡는다"
    points = np.array(points, dtype = float)
    ...
    for cluster_id in set(labels):
        ...
        centroid_distance = np.sqrt(centroid_x**2 + centroid_y**2 + centroid_z**2)
        ## 딕셔너리 객체 리스트 ######### 
        ...
        ##### 각 객체에서 distance 값을 꺼내서 그 값을 기준으로 min을 비교하라. 
        ##### lambda 입력값 : 반환값 즉 결과적으로 가장 거리가 작은 행 전체를 반환한다. 
        ##### cluster_objects = [
        #####{"id": 1, "distance": 2.3},
        #####{"id": 2, "distance": 0.7},
        #####{"id": 3, "distance": 1.5} ]
        ##### 결과 : {"id": 2, "distance": 0.7},
...
def visualize_points(fig, ax, df, labels, x, y, num_detected_obj, cluster_objects, nearest_obj):
    """Point cloud + DBSCAN centroid 시각화."""
    ...
    # 1) 기존 point cloud 유지
    sc = ax.scatter(x, y, s=30, c=labels, cmap="tab20")

    # 2) centroid를 빨간 네모로 overlay
    if cluster_objects:
        centroid_x = [obj["x"] for obj in cluster_objects]
        centroid_y = [obj["y"] for obj in cluster_objects]
        ...
        # 3) centroid 옆에 cluster ID 텍스트 표시
...
def main():
    ...
    try:
        while True:
            ...
            if points:
                df, labels, x, y, filtering_points = dbscanScattering(points)

                if df is None:
                    continue
                cluster_objects, nearest_obj = cluster(filtering_points, labels)
                visualize_points(fig, ax, df, labels, x, y, num_detected_obj, cluster_objects, nearest_obj)
