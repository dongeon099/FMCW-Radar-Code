...
# [실제 차선 판정 폭 설정]
# 각 차선 폭은 X_RANGE * 2 이다.
# 예: X_RANGE = 0.3 이면 왼쪽/중앙/오른쪽 차선 폭은 각각 0.6m이다.
# 차선 판정 폭을 변경하려면 이 값을 수정한다.
X_RANGE = 0.3
...
EGO_SPEED_DEFAULT_KMH = 5.0
EGO_SPEED_DEFAULT_MPS = EGO_SPEED_DEFAULT_KMH / 3.6
...
# =========================
ACC_CRUISE_SPEED_MPS = EGO_SPEED_DEFAULT_MPS
...
# Clean laptop display settings.
# Existing awr111 processing/SPI/ACC settings remain unchanged.
CLEAN_REAR_VIEW_RANGE_M = 30.0

# [비주얼라이저 도로 폭 설정]
# Clean 화면에 그리는 차선 폭이다. 실제 차선 판정 폭과 맞추려면 X_RANGE * 2로 둔다.
# 실제 차선 판정 기준은 위의 X_RANGE와 LEFT/RIGHT_LANE_X_RANGE가 결정한다.
CLEAN_LANE_WIDTH_M = 0.6
CLEAN_SHOW_OBJECT_LABELS = True
CLEAN_EGO_VERTICAL_POSITION = 0.52
CLEAN_EGO_SCALE = 0.55
CLEAN_SPEED_GAUGE_MAX_KMH = 10.0
...
def build_config():
    """실행 설정을 한 곳에서 관리."""
    return {
        "cli_port": "/dev/serial/by-id/usb-Silicon_Labs_CP2105_Dual_USB_to_UART_Bridge_Controller_018AAEC6-if00-port0",
        "data_port": "/dev/serial/by-id/usb-Silicon_Labs_CP2105_Dual_USB_to_UART_Bridge_Controller_018AAEC6-if01-port0",
        "baud_cli": 115200,
        "baud_data": 921600,
        "cfg_file": r"/home/rpi/Documents/awr6843_/profile_2d.cfg",
        "read_timeout": 0.01,
        "cli_timeout": 0.05,
    }
